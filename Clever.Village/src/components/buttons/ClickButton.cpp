...
template <class T>
void ClickButton<T>::setDebounceTicks(int ticks) {
  _debounceTicks = ticks;
} // setDebounceTicks

// explicitly set the number of millisec that have to pass by before a click is detected.
template <class T>
void ClickButton<T>::setClickTicks(int ticks) {
  _clickTicks = ticks;
} // setClickTicks
...
template <class T>
void ClickButton<T>::setPressTicks(int ticks) {
  _pressTicks = ticks;
} // setPressTicks
...
template <class T>
void ClickButton<T>::attachClick(callbackFunction newFunction)
{
...
template <class T>
void ClickButton<T>::attachDoubleClick(callbackFunction newFunction)
{
...
template <class T>
void ClickButton<T>::attachPress(callbackFunction newFunction)
{
...
template <class T>
void ClickButton<T>::attachLongPressStart(callbackFunction newFunction)
{
...
template <class T>
void ClickButton<T>::attachLongPressStop(callbackFunction newFunction)
{
...
template <class T>
void ClickButton<T>::attachDuringLongPress(callbackFunction newFunction)
{
...
template <class T>
bool ClickButton<T>::isLongPressed(){
  return _isLongPressed;
}

template <class T>
int ClickButton<T>::getPressedTicks(){
  return _stopTime - _startTime;
}

template <class T>
void ClickButton<T>::reset(void){
  _state = 0; // restart.
  _startTime = 0;
  _stopTime = 0;
...
template <class T>
void ClickButton<T>::process(void)
{
  ...
  if (_state == 0) { // waiting for menu pin being pressed.
    ...
    if (_doubleClickFunc == NULL || (unsigned long)(now - _startTime) > _clickTicks) {
      ...
      if (_clickFunc) _clickFunc(_context);
      _state = 0; // restart.
